...
def execute_git_command(
    ...
    if not GIT_REPO_PATH or not os.path.isdir(GIT_REPO_PATH):
        raise GitCommandError(
            f"Git repository path '{GIT_REPO_PATH}' is not valid.",
            returncode=-1,
            command=["git"] + command_args,
        )
    ...
    try:
        ...
        raise GitCommandError(
            f"Git command timed out after {timeout} seconds.", command=full_command
        )
...
def get_status() -> Dict[str, Any]:
    ...
    staged_stdout, _ = execute_git_command(
    ...
    untracked_stdout, _ = execute_git_command(
...
def unstage_all_files() -> Tuple[str, str]:
    ...
    staged_files_stdout, _ = execute_git_command(["diff", "--name-only", "--cached"])
    if not staged_files_stdout:
        return "No files to unstage.", ""

    # The command `git restore --staged .` unstages all files in the current directory.
    return execute_git_command(["restore", "--staged", "."])
...
def commit_changes(message: str) -> Tuple[str, str]:
    ...
    staged_files_stdout, _ = execute_git_command(
...
def pull_remote_changes(
    ...
    remote_to_use, branch_to_use = (
        remote or GIT_REMOTE_NAME,
        branch or GIT_DEFAULT_BRANCH,
    )
    ...
    logger.info(
        f"Pulling changes from remote '{remote_to_use}', branch '{branch_to_use}'"
    )
    ...
    try:
        ...
        if "fatal: Authentication failed" in stderr:
            raise GitCommandError(
                "Authentication failed. Please check your SSH key setup.", stderr=stderr
            )
...
def push_local_changes(
    remote: Optional[str] = None, branch: Optional[str] = None, force: bool = False
) -> Tuple[str, str]:
    remote_to_use, branch_to_use = (
        remote or GIT_REMOTE_NAME,
        branch or GIT_DEFAULT_BRANCH,
    ...
    logger.info(
        f"Pushing local changes to remote '{remote_to_use}', branch '{branch_to_use}'"
    )
    ...
    try:
        ...
        if "failed to push some refs" in stderr:
            ...
            raise GitCommandError(
                "Push rejected. Tip: Try pulling remote changes first.", stderr=stderr
            )
...
def get_remote_url(remote_name: Optional[str] = None) -> Optional[str]:
    try:
        return execute_git_command(
            ["remote", "get-url", remote_name or GIT_REMOTE_NAME], check=True
        )[0]
    except GitCommandError:
        return None
...
def sync_workspace(commit_message: str) -> Tuple[str, str]:
    ...
    try:
        ...
        raise GitCommandError(
            "Sync failed: Could not stage changes.", stderr=e.stderr
        ) from e
    ...
    try:
        ...
        raise GitCommandError(
            "Sync failed: Could not commit changes.", stderr=e.stderr
        ) from e

    try:
        pull_stdout, pull_stderr = pull_remote_changes()
    ...
    try:
        push_stdout, push_stderr = push_local_changes()
        logger.info("Sync: Push to remote successful.")
    except GitCommandError as e:
        logger.error(f"Sync failed at 'push' stage: {e}")
        raise GitCommandError(
            "Sync failed: Could not push local changes.", stderr=e.stderr
        ) from e
