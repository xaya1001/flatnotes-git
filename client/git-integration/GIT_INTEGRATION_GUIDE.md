...
For security. We need to provide the application with an SSH key to authenticate with your Git provider (e.g., GitHub). Instead of just mounting your entire `.ssh` directory, which can be risky, we use a safer method: you mount a *specific* key and your `known_hosts` file, and the application copies them into the container with strict, secure permissions. This prevents accidental exposure of your other keys and ensures the file permissions are correct inside the Docker container.
...
```
*(Replace `github.com` if you use GitLab, Bitbucket, etc.)*
